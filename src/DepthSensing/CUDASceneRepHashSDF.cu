...
//    }
    ...
    if (screenPos.x < cameraParams.m_imageWidth && screenPos.y < cameraParams.m_imageHeight) {	//on screen
        ...
        if(cameraData.d_maskData)// && cameraData.d_objectData
        {
            ...
            if(cameraData.d_objectData!=NULL)
            {
                objLabel = cameraData.d_objectData[cameraParams.m_imageWidth * catY + catX];
            }
            objectIndex = 0;//cameraData.d_objectData[cameraParams.m_imageWidth * catY + catX];
        }
...
//                        }
                        ...
                        //
                        if(personLabel == 255 || objLabel == 255)
                        {
                            newVoxel = oldVoxel;
                        }
                       else{
