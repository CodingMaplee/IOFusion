...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename, const uchar* personMask, const uchar* dynamicMask)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
   ...
   if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray, imDepth, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, personMask, dynamicMask, mpCamera);
