...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const uchar* personMask, const uchar* dynamicMask, GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib &ImuCalib)
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    for (size_t i(0); i < mvKeys.size(); ++i)
    {
        ...
        uchar val = personMask[(uint)(mvKeys[i].pt.y + 0.5) * FRAME_WIDTH + (uint)(mvKeys[i].pt.x + 0.5)];
        uchar val2 = dynamicMask[(uint)(mvKeys[i].pt.y + 0.5) * FRAME_WIDTH + (uint)(mvKeys[i].pt.x + 0.5)];
